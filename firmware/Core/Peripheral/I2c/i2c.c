...
i2c_read_8_bit(i2c_dev_t *dev, uint16_t reg_addr, uint8_t *out)
{
    ...
    HAL_StatusTypeDef ret2 = HAL_I2C_Master_Receive(&dev->i2c_handle, dev->dev_addr | 1, out, 1, I2C_TIMEOUT);
    return ret1 & ret2;
}
...
i2c_read_16_bit(i2c_dev_t *dev, uint16_t reg_addr, uint16_t *out)
{
    ...
    HAL_StatusTypeDef ret1 = HAL_I2C_Master_Transmit(&dev->i2c_handle, dev->dev_addr, data, 2, I2C_TIMEOUT);
    HAL_StatusTypeDef ret2 = HAL_I2C_Master_Receive(&dev->i2c_handle, dev->dev_addr | 1, out_data, 2, I2C_TIMEOUT);
    *out = out_data[0];
    *out <<= 8;
    *out |= out_data[1];
    return ret1 & ret2;
